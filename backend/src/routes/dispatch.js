...
const dispatchValidation = [
  ...
  body('random_tonnage').optional({ checkFalsy: true }).isFloat({ min: 0 }).withMessage('Random tonnage must be >= 0'),
  body('random_pieces').optional({ checkFalsy: true }).isInt({ min: 0 }).withMessage('Random pieces must be >= 0'),
